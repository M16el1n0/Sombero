...
COOLDOWN_FILE = "task_cooldown.json"    # глобальный КД (один на всех)
...
def load_platforms() -> dict:
    """Дефолтные платформы всегда присутствуют. Старые дефолтные — удаляются. Кастомные сохраняются."""
    saved = {}
    ...
    # Берём только кастомные (тех, кого нет в DEFAULT_PLATFORMS)
    custom = {name: price for name, price in saved.items() if name not in DEFAULT_PLATFORMS}
    # Дефолтные всегда первыми, потом кастомные
    merged = dict(DEFAULT_PLATFORMS)
    merged.update(custom)
    save_platforms(merged)
    return merged
...
def set_cooldown():
    ...
    with open(COOLDOWN_FILE, "w", encoding="utf-8") as f:
        json.dump({"last_post": datetime.now().isoformat()}, f, ensure_ascii=False)
...
async def start_task_creation(query, context):
    ...
    if remaining > 0:
        ...
        secs = remaining % 60
        await query.answer(
            f"⏳ КД активен — ещё {mins} мин {secs} сек до следующего поста.",
            show_alert=True
        )
        return MAIN_MENU
...
async def confirm_task(query, context):
    ...
    buttons = [
        [
            ...
            InlineKeyboardButton("Не могу написать", callback_data="cant_write"),
        ],
    ]
    if URL_HOW_TO != "https://ВАШ_ЛИНК":
        buttons.append([InlineKeyboardButton("Как брать задания?", url=URL_HOW_TO)])
...
async def handle_close_task(query, context):
    ...
    try:
        ...
        closed_text = (
            "🔒Данное задание закончилось. Скоро будут новые!\n"
            "Включайте уведомления в канале, чтобы не пропустить новые задания!"
        )
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            TASK_PLATFORM:      [CallbackQueryHandler(button_callback)],
    ...
    app.add_handler(CallbackQueryHandler(cant_write_callback, pattern="^cant_write$"))
