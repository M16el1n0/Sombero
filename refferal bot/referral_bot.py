...
WEBSITE_URL     = "https://m16el1n0.github.io/sayt_sombero/"            # ← замени на свой сайт
...
def profile_text(uid, invite_url):
    user = get_user(uid)
    ...
    link_line = (
        ...
        if invite_url else
        "⚠️ Не удалось создать ссылку. Бот должен быть администратором канала."
    )
    ...
    return (
        ...
        f"{withdraw_hint}\n\n"
        f"🔗 <b>Ваша реферальная ссылка на канал:</b>\n"
        f"{link_line}"
    )
...
async def cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ══════════ ПРОФИЛЬ ══════════
        ...
        invite_url = await get_or_create_invite(uid, context)
        await query.edit_message_text(
            profile_text(uid, invite_url),
            parse_mode="HTML",
